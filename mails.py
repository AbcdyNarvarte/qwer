...
from global_func import on_show, handle_logout, create_email
...
class MessagesPage(tk.Frame):
    ...
    def user_information(self):
        ...
        try:
            conn = sqlite3.connect('main.db')
            c = conn.cursor()
            fetch = c.execute("SELECT user_id, f_name, l_name, useremail, phonenum  FROM users WHERE user_id = ?", (user_id,)).fetchone()

            if fetch:
                user_info = f"User ID: {fetch[0]}\nFirst Name: {fetch[1]}\nLast Name: {fetch[2]}\nEmail: {fetch[3]}\nPhone Number: {fetch[4]}"
            else:
