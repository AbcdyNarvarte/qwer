...
if not product_logger.handlers:
    product_handler = logging.FileHandler('C:/capstone/log_f/product.log')
    ...
    def create_product(self):
        ...
        try:
            ...
            if self.session and 'user_id' in self.session:
                ...
                try:
                    # Log to user_logs table in database
                    ...
                    # Log to user_activity.log using standardized logger
                    from log_f.user_activity_logger import user_activity_logger
                    user_activity_logger.log_activity(
                        user_id=user_id,
                        username=user_name,
                        action=f"Created product '{product_name}' (ID: {product_id})",
                        feature="product",
                        operation="create",
                        details=f"Materials: {', '.join(self.current_materials)}"
                    )
    ...
    def edit_product(self, product_id, product_name, materials):
        ...
        def save_changes():
            ...
            try:
                ...
                # Log the product edit to product.log and user_activity.log
                if self.session and 'user_id' in self.session:
                    user_id = self.session.get('user_id')
                    user_name = self.session.get('f_name', self.session.get('username', 'Unknown'))
                    ...
                    user_activity_logger.log_activity(
                        user_id=user_id,
                        username=user_name,
                        action=f"Updated product '{product_name}' to '{new_name}' (ID: {product_id})",
                        feature="product",
                        operation="update",
                        details=f"Materials updated"
                    )
...
pady=8)
    ...
    def delete_product(self, product_id, product_name):
        ...
        try:
            ...
            if messagebox.askyesno("Confirm Deletion", 
                ...
                # Log the deletion to product.log and user_activity.log
                if self.session and 'user_id' in self.session:
                    user_id = self.session.get('user_id')
                    user_name = self.session.get('f_name', self.session.get('username', 'Unknown'))
    ...
    def show_product_list(self):
        ...
        columns = ('ID', 'Name', 'Materials', 'Created Date', 'Status Quo', 'Created By')
        ...
        product_tree.heading('Created By', text='Created By')
        ...
        product_tree.column('Created By', width=150, minwidth=120)
        ...
        def approve_selected_product():
            ...
            try:
                ...
                if unavailable_mats:
                    ...
                    for mat_name, mat_qty in selected_product['materials'].items():
                        c.execute("UPDATE raw_mats SET current_stock = current_stock - ? WHERE mat_name = ?", 
                                 (mat_qty, mat_name))
            ...
            if hasattr(self, 'controller') and self.controller:
                ...
                # Show inventory frame after refresh
                self.controller.show_frame(FrameNames.INVENTORY)
                
            # Show the order list
            self.window.after(500, self.show_order_list)
